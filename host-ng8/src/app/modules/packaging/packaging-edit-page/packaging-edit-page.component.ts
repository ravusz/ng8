...
export class PackagingEditPageComponent implements OnInit {
  ...
  constructor(private packagingService: PackagingService, private router: Router, private route: ActivatedRoute, private messageService: MessageService) { }
  packaging: Packaging;

  ngOnInit() {
    this.packagingService.getById(this.route.snapshot.paramMap.get('id')).subscribe(data => {
      this.packaging = data;
    }, error => {
  ...
  onSubmit(packaging: Packaging) {
    this.packagingService.update(this.route.snapshot.paramMap.get('id'), packaging).subscribe(data => {
      this.messageService.add({
        severity: 'success',
